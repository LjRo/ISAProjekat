...
public interface RentCarRepository extends JpaRepository<RentACar, Long> {
    ...
    @Query(value = "SELECT * FROM rentacar rc \n" +
            "\t\tWHERE EXISTS (SELECT * FROM rent_office ro left outer join locations l on ro.location_id = l.id WHERE l.address_name LIKE CONCAT('%',?1,'%') AND ro.rentacar_id = rc.id) \n" +
            "        AND EXISTS\t(SELECT * FROM cars as c WHERE c.type_id = 1 AND c.rentACar_id = rc.id AND c.id NOT IN\n" +
    ...
    @Query(value = "SELECT * FROM rentacar rc WHERE rc.name like CONCAT('%',?1,'%') AND EXISTS\t(SELECT * FROM cars as c WHERE c.type_id = 2 AND c.rentACar_id = rc.id AND c.id NOT IN\n" +
            "\t\t\t\t\t(SELECT r.rented_car_id \n" +
            "\t\t\t\t\t FROM rent_reservation as r \n" +
